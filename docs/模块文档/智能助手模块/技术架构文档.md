...
后台「AI 模型管理」中每个模型可设置代理类型；用户发起对话时，Java 根据该模型的 `proxy_type` 选择对应代理 URL 发起请求。详见 [部署文档 - AI 代理部署](../../运维部署/部署文档.md#7-ai-代理部署python—-国内--海外双机)。
...
生产环境建议按 [部署文档 - AI 代理双机部署](../../运维部署/部署文档.md#7-ai-代理部署python—-国内--海外双机) 在国内、海外各部署一台 ai-proxy。单机开发可仅起一个服务：
