...
系统支持以下厂商模型，管理员可在后台「AI 模型管理」中配置对应 API Key 与模型标识，供用户在前端选择使用。
...
| **通义千问**（Qwen / 阿里云） | `qwen` | `https://dashscope.aliyuncs.com/compatible-mode/v1` | ✅ |
| **智谱 GLM**（Zhipu） | `zhipu` | `https://open.bigmodel.cn/api/paas/v4` | ✅ |

模型名称支持前缀格式（如 `mistral:mistral-large-latest`）或由代理自动推断。
